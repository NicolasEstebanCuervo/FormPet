...
function App() {
    ...
    const agregarTarea = (tarea) => {
        ...
        } else if (tarea.tipo == "Hamster" || tarea.tipo == "hamster") {
            tarea.emojiMascota = "1f439";
        } else if (tarea.tipo == "Conejo" || tarea.tipo == "conejo") {
    ...
    return (
        <div
            className={
                color
                    ? "App texto_blanco modo_oscuro"
                    : "App texto_gris modo_claro"
            }
        >
                ...
                <Nav
                    fondoNav={
                        !hidden
                            ? "hidden"
                            : color
                            ? "modo_oscuro_2 texto_blanco"
                            : " modo_claro_2 texto_gris"
                    }
                    colorTexto={color ? "texto_blanco" : "texto_gris"}
                    colorToggle={
                        color ? "modo_claro_toggle" : "modo_oscuro_toggle"
                    }
                    itemsNav={
                        color ? "modo_oscuro_2_items" : "modo_claro_2_items"
                    }
                    setNombre={
                        mostarInput ? (
                            <input
                                ...
                                className={
                                    color
                                        ? "input_oscuro focus_oscuro input_nombre"
                                        : "input_claro focus_claro input_nombre"
                                }
                                onKeyUp={input_enter}
                ...
                <Formulario
                    ...
                    inputNombre={
                        hiddenNombre
                            ? "hidden"
                            : color
                            ? "input_focus_oscuro texto_blanco"
                            : "input_focus_claro"
                    }
                    colorInput={
                        color
                    ...
                    botonEnviar={
                        color
                            ? "boton_enviar modo_oscuro_boton"
                            : "boton_enviar modo_claro_boton"
                    }
